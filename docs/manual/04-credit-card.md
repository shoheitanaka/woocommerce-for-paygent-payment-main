...
- **「与信決済」を選ぶ場合の重要な注意**: 「与信決済」はお客様のカードに与信枠を確保するだけで、この時点ではまだ代金は確定していません。**実際に代金を確定（売上確定）させるには、WooCommerceの注文ステータスを手動で「完了」に変更する必要があります。** 注文ステータスを「完了」にした瞬間に、ペイジェント側へ売上確定の指示が自動的に送られます。逆に言うと、**注文ステータスを「処理中」のままにしていると、商品を発送してもお客様への課金は行われません。** 発送作業だけで満足せず、必ず注文ステータスを「完了」に変更し、[注文の管理](08-order-management.md) で紹介した注文メモで売上確定の結果を確認する運用を徹底してください。仕組みが複雑なため、慣れないうちは前述の通り「即時売上」を選ぶことをおすすめします。
