...
- **決済状況**: 「即時売上」（注文と同時に代金を確定する）か、「与信決済」（与信枠を確保するだけで、後日あらためて売上確定処理を行う）かを選びます。**初めての場合は「即時売上」を強くおすすめします。**
...
- **「与信決済」を選ぶ場合の重要な注意**: 「与信決済」はお客様のカードに与信枠を確保するだけで、この時点ではまだ代金は確定していません。**実際に代金を確定（売上確定）させるには、WooCommerceの注文ステータスを手動で「完了」に変更する必要があります。** 注文ステータスを「完了」にした瞬間に、ペイジェント側へ売上確定の指示が自動的に送られます。逆に言うと、**注文ステータスを「処理中」のままにしていると、商品を発送してもお客様への課金は行われません。** **注意: 売上確定に失敗した場合は注文メモにエラーが記録されますが、成功した場合は注文メモに記録が残りません。**「完了」に変更した後、注文メモにエラーが出ていないかを確認し、さらに [注文の管理](08-order-management.md) で紹介するペイジェント加盟店管理者サイトでも売上確定の結果を確認することをおすすめします。仕組みが複雑なため、慣れないうちは前述の通り「即時売上」を選ぶことをおすすめします。
