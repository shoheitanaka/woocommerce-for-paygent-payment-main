...
設定はシンプルですが、**Paidy・楽天ペイ・キャリア決済は、お客様の支払い操作が終わった時点では「支払いの承認（オーソリ）」までしか行われていません**（[PayPay](06-paypay.md) も同様です）。注文は「処理中」になりますが、この時点ではまだ売上が確定していない状態です。

商品の発送などが済んだら、必ず注文ステータスを「完了」に変更してください。この操作をきっかけに、ペイジェントへ各決済手段の売上指示が送られます。「完了」に変更したあとは、注文詳細画面の「注文メモ」に売上処理の成功が記録されていることを確認してから、その注文の売上を確定済みとして扱ってください。失敗のメモが記録されている場合は売上が確定していませんので、エラー内容を確認のうえ再度操作するか、ペイジェント社にお問い合わせください。
...
> **既知の問題（同じ商品を2個以上購入する注文）**: 執筆時点のバージョンでは、**同じ商品を数量2以上で購入する注文があると、ペイジェントへ送信される商品内訳の金額が正しく計算されません**（内訳の合計が実際の請求額と食い違った状態で送信されます）。このため、申込がエラーになったり、楽天ペイの支払い画面に正しくない内訳が表示されたりする可能性があります。同じ商品を複数個まとめて購入されることが多い店舗では、この問題が解消されるまで楽天ペイの利用は慎重にご検討ください。この制限は将来のアップデートで解消される可能性があります。

### 銀行ネット

インターネットバンキングを利用したオンライン振込決済です。

> **設定の注意（文字種）**: 銀行ネットの設定タブには、請求書に表示する「請求書詳細」「店舗名」の項目が漢字用・カナ用でそれぞれ用意されています。**カナ欄は半角カタカナのみ、漢字欄は全角文字のみ**を入力してください（共通の設定手順で触れた「タイトル」「説明」などとは別に、この4項目は必ず個別に確認・入力する必要があります）。全角文字をカナ欄に入力する、または半角文字を漢字欄に入力するなど、文字種を間違えたまま保存すると、ペイジェント側で申込が正しく処理されずエラーになります。
...
> **設定の注意（文字種）**: ATM決済の設定タブには、請求書に表示する「請求書詳細（カナ）」の項目があります。**この欄は半角カタカナのみ**を入力してください（共通の設定手順で触れた「タイトル」「説明」などとは別に、この項目は必ず個別に確認・入力する必要があります）。初期値のままにしていたり、全角文字を入力したまま保存すると、ペイジェント側で申込が正しく処理されずエラーになります。
...
> **対応通貨についての注意**: この決済手段が実際に利用できるのは、ストアの通貨設定（WooCommerceの「設定」→「一般」の「通貨」）が以下のいずれかになっている場合です。
>
> USD, EUR, GBP, KRW, CNY, TWD, HKD, SGD, AUD, CAD, DKK, INR, MYR, NOK, PHP, RUB, VND, SEK, CHF, THB, BRL, IDR, AED
>
> **日本円（JPY）を設定している場合は自動的に非表示になりますが、上記リストにない他の通貨（NZD、MXNなど）を設定した場合は自動的には非表示になりません。** この場合、チェックアウト画面には表示され続けるものの、実際に決済しようとするとペイジェント側でエラーになり失敗します。上記リストにない通貨でストアを運営する場合は、この決済手段を有効化しないでください。
