...
- **接続IPアドレス**: サーバー内部から見えるIPアドレスが自動表示されます。**この画面に表示される値は、Paygent社に登録すべき「外部から見た発信元IPアドレス」と異なる場合があります。** 特にレンタルサーバーやクラウド環境（NAT・ロードバランサー・コンテナ環境など）では、この画面には`172.18.0.4`のような内部用の非公開アドレスが表示されることが多く、そのまま登録するとP015エラーの原因になります。ペイジェント社の申込書に登録するIPアドレスは、この画面の表示値を鵜呑みにせず、契約しているサーバー会社に「外部への発信元IPアドレス（Paygent社に登録すべきIPアドレス）」を直接確認してから登録してください。
...
- **認証済みCAファイル**: **この項目は必須です。** ペイジェントのサーバーとの通信を安全に検証するためのCA証明書ファイル（`.crt`形式）をアップロードしてください。このファイルが設定されていないと、テスト環境・本番環境を問わず**すべての決済通信が失敗します**。多くの場合、ご利用のサーバー会社（レンタルサーバー）が提供する標準的なCA証明書バンドルファイルで動作します。入手方法が分からない場合は、サーバー会社のサポート窓口にお問い合わせいただくか、ペイジェント社にご相談ください。
- **試験環境**: **新規インストール直後は、この項目はチェックが入っていません。** 決済のテストを行う前に、必ず自分でチェックを入れて「変更を保存」してください。チェックを入れると、テスト環境用の認証情報で動作するようになります（詳しくは [テスト環境での動作確認の進め方](09-testing-overview.md) で解説します）。**このチェックを入れ忘れたままテストすると、本番用の認証情報（未設定の場合は空）で動作しようとしてしまう**ため、テスト注文を行う前に必ず設定を確認してください。
